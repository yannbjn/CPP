...
/*   Updated: 2024/09/23 18:01:58 by yabejani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::setAD(int AD){
	...
	std::cout << UNDERLINE << GREEN << this->_name << RESET << " Attack Damage has been set to " << BOLD << YELLOW << this->_AttackDamage << RESET << "." << std::endl;
}

void	ClapTrap::getInfo(){
	std::cout << UNDERLINE << GREEN << this->_name << RESET << BOLD << YELLOW << " has:" << std::endl;
...
void	ClapTrap::takeDamage(unsigned int amount){
	...
	std::cout << UNDERLINE << GREEN << this->_name << RESET << BOLD << RED << " took " << amount << " damage after being attacked!" << RESET << std::endl;
	ClapTrap::getInfo();
}

void	ClapTrap::beRepaired(unsigned int amount){
	if (this->_EnergyPoints <= 0)
		std::cout << UNDERLINE << GREEN << this->_name << RESET << BOLD << MAGENTA << " can't be repaired without Energy Points." << RESET << std::endl;
	if (this->_HitPoints <= 0)
		std::cout << UNDERLINE << GREEN << this->_name << RESET << BOLD << MAGENTA << " can't be repaired without Hit Points." << RESET << std::endl;
	else if (this->_EnergyPoints > 0 && this->_HitPoints > 0){
		this->_EnergyPoints--;
		this->_HitPoints += amount;
		std::cout << UNDERLINE << GREEN << this->_name << RESET << BOLD << BLUE << " got repaired for " << amount << " Hit Points." << RESET << std::endl;
	}
	ClapTrap::getInfo();
}
